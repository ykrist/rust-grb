...
fn build_cycles(edges_used: impl IntoIterator<Item=(usize, usize)>) -> Vec<Vec<(usize, usize)>> {
  ...
  let mut neighbours: FnvHashMap<usize, Vec<usize>> = (0..N).map(|i| (i, Vec::with_capacity(2))).collect();
    ...
    loop {
      ...
      let (next, next_neigh) = if let Some(next_neigh) = neighbours.remove(&n1) {
        ...
        if let Some(next_neigh) = neighbours.remove(&n2) {
          ...
          cycle.push(if current < start { (current, start) } else { (start, current) });
          break;
        }
      };

      cycle.push(if current < next { (current, next) } else { (next, current) });
      neigh = next_neigh;
      current = next;
    }
...
fn get_edges_used(model: &Model, xvars: &FnvHashMap<(usize, usize), Var>) -> grb::Result<Vec<(usize, usize)>> {
...
impl<'a> SubtourElimination<'a> {
  pub fn new(edge_vars: &'a FnvHashMap<(usize, usize), Var>) -> SubtourElimination<'a> {
    SubtourElimination { edge_vars, subtours_found: 0 }
  }
}
...
impl callback::Callback for SubtourElimination<'_> {
  fn callback(&mut self, w: Where) -> callback::CbResult {
    if let Where::MIPSol(ctx) = w {
      let edges_used: Vec<_> = self.edge_vars.keys()
      ...
      if cycles.len() > 1 {
        ...
        for cycle in cycles {
          ...
          ctx.add_lazy(c!(cycle.iter().map(|e| self.edge_vars[e]).grb_sum() <= cycle.len() - 1))?;
...
fn fmt_cycle(cycle: &Vec<(usize, usize)>) -> String {
  let order : Vec<_> = cycle_to_node_order(cycle).into_iter().map(|i| format!("{}", i)).collect();
  return order.join(" -> ")
}
...
fn main() -> grb::Result<()> {
  ...
  for i in 0..N {
    for j in i + 1..N {
      ...
      x.insert((i, j), add_binvar!(model, obj: TRAVEL_TIMES[i * N + j], name: &format!("X[{},{}]", i, j))?);
    }
  }
